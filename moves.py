...
def shooting_move(ens, level=0):
    ...
    if ptype in ens.illegal_pathtypes:
        logger.info("Illegal pathtype {} for {}".format(ptype, ens.name))
        return "ILL", new_path
    ...
    if not ens.check_cross(new_path):
        ...
        assert ens.check_path(new_path), "ACC path not acceptable?!"
        return "ACC", new_path
...
def cut_LR_to_M(ens, reverse, level=0, M=None, path=None):
    ...
    if ens.ens_type in ['PPTIS_0plusmin_primed', '0R']:
        M = ens.intfs['L']
    elif ens.ens_type in ['PPTIS_Nplusmin_primed', 'NL']:
        M = ens.intfs['R']
    else:
        M = ens.intfs['M']

    if path is None:
        path = ens.paths[level]

    if reverse == -1:
        # extract shootpoint
        shootpoint = path.phasepoints[0]
        shootpoint_op = path.orders[0]
        # are we left or right of M?
        right_of_M = True if shootpoint_op[0] > M else False
        # start cutting
        phs, ops = [], []
        for ph, op in zip(path.phasepoints[1:],
                          path.orders[1:]):
            phs.append(ph)
            ops.append(op)
        ...
        # extract shootpoint
        shootpoint = path.phasepoints[-1]
        shootpoint_op = path.orders[-1]
        # are we left or right of M?
        right_of_M = True if shootpoint_op[0] > M else False
        # start cutting
        phs, ops = [], []
        for ph, op in zip(reversed(path.phasepoints[:-1]),
                          reversed(path.orders[:-1])):
            phs.append(ph)
            ops.append(op)
            if right_of_M and op[0] < M:
                break
...
def cut_extremal_phasepoints(ens, reverse, level=0, path=None):
    ...
    if path is None:
        path = ens.paths[level]
    if reverse == -1:
        ph_overlap = [path.phasepoints[i] for i in [0, 1]]
        op_overlap = [path.orders[i] for i in [0, 1]]
        sh = path.phasepoints[0]
    elif reverse == 1:
        ph_overlap = [path.phasepoints[i] for i in [-2, -1]]
        op_overlap = [path.orders[i] for i in [-2, -1]]
        sh = path.phasepoints[-1]
    else:
        raise ValueError("reverse must be either 1 or -1")
